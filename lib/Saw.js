...
class Saw {
	...
	match (match) {
		...
		matchArray.some(match => {
			if (typeof match === 'string') {
				...
				match = escapeStringRegexp(String(match || ''));
			}
	...
	matchAll(match) {
		...
		for (let string of array) {
			[
				...
				results.push(match.groups ? match.groups : match.slice(1));
			})
		}
	...
	slice (begin, end) {
		...
		let context = Array.isArray(saw._context) ? saw._context : this._contextToString(saw._context);
	...
	toFloat () {
		...
		if (isNaN(result) || (string).replace(/\.0+$/, '').length != String(result).length) {
			return NaN;
		} else {
			return result;
		}
	...
	toObject () {
		...
		if (arguments.length) {
			...
			object = this._context.matches.groups;
		}
	...
	startsWith (match) {
		let saw = new Saw(this._context),
			array = saw.toArray(),
			regExp = match ? new RegExp(`^${escapeStringRegexp(String(match || ''))}`) : null,
			matches = array.length ? true : false;
	...
	indexesOf (match) {
		...
		if (Array.isArray(this._context)) {
			...
			let pattern = new RegExp(
				match instanceof RegExp ? match.source : escapeStringRegexp(String(match || '')),
